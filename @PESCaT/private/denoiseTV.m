function [x , J] = denoiseTV(y,lambda,Nit,ctype) %J
...
%sosy=sos(y);
%sosx=sos(x);
%sosx=abs(sosx)/max(abs(sosx(:)));
%sosy=abs(sosy)/max(abs(sosy(:)));
%sosy = imadjust(sosy);
%sosx = imhistmatch(sosx,sosy);

%figure;imshow(cat(2,sosy,sosx),[],'InitialMagnification',200);title(['input-output ctype = ' num2str(ctype)])
%
return
