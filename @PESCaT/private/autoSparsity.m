function [best_lambda,res,nextRange] = autoSparsity(f1,f0,n,sigma,lmbd,cdata,mask)
%
...
sx2 = 2^pwr;
% Make the data, consistency data, mask and segmentation maps power of 2
...
for k=1:length(lmbd)
    ...
    Res(:,:,k)=tmp_res;
    dt = max(abs(hn1(:)))*1e-6; % Threshold for degree of freedom. Set as 10^-6 of max of intermediate images
...
mmp = min(lmp,ump); % Minimum of midpoint lengths 
nextRange = linspace((lmbd(1)+mmp),(lmbd(end)-mmp),100);